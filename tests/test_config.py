...
class TestGlobalSettings:
    def test_defaults(self) -> None:
        ...
        assert g.summary_direction == "outbound"

    def test_from_dict(self) -> None:
        g = GlobalSettings.from_dict({"check_interval": 120, "audio": False})
        assert g.check_interval == 120
        assert g.audio is False
        assert g.margin_below == 30.0

    def test_from_dict_ignores_unknown(self) -> None:
        g = GlobalSettings.from_dict({"check_interval": 60, "unknown_field": "ignored"})
        assert g.check_interval == 60
...
class TestWatchTarget:
    ...
    def test_resolve_balance_defaults_to_class_default(self) -> None:
        w = WatchTarget(customer="18")
        assert w.resolve_balance_below() == -500.0

    def test_resolve_billed_above(self) -> None:
...
class TestSettings:
    ...
    def test_validate_missing_balance(self) -> None:
